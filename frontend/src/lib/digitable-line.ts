...
export function formatDigitableLine(digits: string): string {
  if (digits.length === BANK_SLIP_LENGTH) {
    return `${digits.slice(0, 5)}.${digits.slice(5, 10)} ${digits.slice(10, 15)}.${digits.slice(15, 21)} ${digits.slice(21, 26)}.${digits.slice(26, 32)} ${digits[32]} ${digits.slice(33)}`;
  }
  if (digits.length === COLLECTION_LENGTH) {
    return [0, 1, 2, 3]
      .map((index) => {
        const block = digits.slice(index * 12, index * 12 + 12);
        return `${block.slice(0, 11)}-${block[11]}`;
      })
      .join(' ');
  }
  return digits;
